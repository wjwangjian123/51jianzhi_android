...
import com.part.jianzhiyi.modulemerchants.app.MyApp;
    ...
    protected <M> ResultObserver getResult(ResultObserver<M> resultObserver) {
        ResultObserver<M> success = new ResultObserver<M>() {
            ...
            public void onError(Throwable e) {
                super.onError(e);
                if (!NetworkUtils.checkNetworkConnect(MyApp.context())) {
                    Toast.makeText(MyApp.context(), "请检查您的网络哦", Toast.LENGTH_SHORT).show();
                } else if (NetworkUtils.isWifiProxy(MyApp.context())) {
                    Toast.makeText(MyApp.context(), "请关闭您的代理哦", Toast.LENGTH_SHORT).show();
                } else {
